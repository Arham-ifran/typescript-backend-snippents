import { BlockchainNetworks } from '@xyz/database'
import { AlertPriority, ContractType } from '@xyz/utils'
import { getSubscriber } from '@xyz/web3'
...
/**
 * The subscribe method subscribes to all the events emitted from the xyz Token Smart Contracts, and the data is stored in the
...
export const subscribe = async (): Promise<void> => {
  const { subscription: tokenSubscriber, contract } = await subscriber(
    ContractType.xyzToken,
    BlockchainNetworks.Ethereum,
  )

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  tokenSubscriber.on('connected', (connected: any) =>
    logger.info(`xyz token connected: ${JSON.stringify(connected)}`),
  )
  ...
  tokenSubscriber.on('error', async (error: any) => {
    const message = `Moderate: Web3 subscriber error for contractType: ${ContractType.xyzToken} contract and network: ${BlockchainNetworks.Ethereum}.`
